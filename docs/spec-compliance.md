...
| Request body parsing as PSR-15 middleware | ✅ test | `Middleware\RequestBodyParsingMiddleware` forces an early JSON decode (malformed → `RequestBodyInvalidJson` → 400) and validates the top-level member rules (`data`/`errors`/`meta`, both → `TopLevelMembersIncompatible` → 400), delegating both to `Negotiation\RequestValidator`; bodyless requests untouched. `RequestBodyParsingMiddlewareTest`. |
