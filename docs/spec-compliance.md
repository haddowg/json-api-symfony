...
| Top-level `jsonapi.profile` carries applied profile URIs | ✅ test | Populated by the response render path from the applied-profile set. `ProfileApplicationTest`. |
...
| Cursor-pagination profile (first consumer) | ✅ test | `Pagination\CursorPaginationProfile` (`…/ethanresnick/cursor-pagination/`); `CursorBasedPage` activates it, so cursor-paginated responses advertise it on `Content-Type` + `jsonapi.profile`. `DataResponsePaginationTest::cursorResponseOmitsLastAndAdvertisesTheCursorProfile`. |
