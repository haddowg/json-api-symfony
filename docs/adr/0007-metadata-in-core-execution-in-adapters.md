...
*metadata*: `ConstraintInterface`, `FilterInterface`, and `SortInterface` carry no `apply()` or `validate()`
...
A reader expecting `FilterInterface::apply($query)` will not find it — this split is what
