...
| Area | Decision |
|---|---|
| Namespace structure | Yin's current layout has supporting types under `Schema\*` (`Schema\Document\*`, `Schema\Resource\*` for the serializer class, `Schema\Link\*`, `Schema\Error\*`, `Schema\Pagination\*`, `Schema\Relationship\*`). Phase 4.5 introduces a top-level fluent `Schema` type; the existing `Schema\*` subdirectories move to `Document\*` for clarity, since the contents are document parts. The rename is for readability rather than to resolve a hard collision. |
...
| 3 | PSR-15 middleware suite | [phase-3-middleware.md](./phase-3-middleware.md) | Ready |
| 4 | Validation | [phase-4-validation.md](./phase-4-validation.md) | Not started |
| 4.5 | Fluent schema DSL | [phase-4-5-fluent-schema.md](./phase-4-5-fluent-schema.md) | Not started |
| 5 | Docs port & update | [phase-5-docs.md](./phase-5-docs.md) | Not started |
