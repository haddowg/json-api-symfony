 ...
 * }
 * @psalm-type JsonApiConfig = array{
 *     base_uri?: scalar|Param|null, // Default: ""
 *     version?: scalar|Param|null, // Default: "1.1"
 *     schema_validation?: bool|Param, // Validate write bodies against the JSON:API JSON Schema (requires opis/json-schema). A dev/CI structural linter, separate from the always-on Symfony Validator bridge. // Default: false
 *     max_include_depth?: int|Param, // Cap the nesting depth of a `?include` request (number of relationship hops from the primary resource): a cap of 3 allows `?include=a.b.c` and rejects `a.b.c.d` (400). The default is 3; set 0 for unlimited. A resource's own maxIncludeDepth() overrides this server default. // Default: 3
 *     strict_query_parameters?: bool|Param, // Reject an unrecognized top-level query-parameter family with a 400 (bundle ADR 0055, core ADR 0059). A param is recognized when its base name is a reserved JSON:API family (include/fields/filter/sort/page), a key the primary resource declares, a negotiated profile keyword (relatedQuery/rQ for the Relationship Queries profile, withCount for the Countable profile), or an app-registered custom param. The default is true; set false to restore the old silent-ignore behaviour. // Default: true
 *     exceptions?: array<string, int|Param>,
 *     error_codes?: array{ // The error codes your application documents alongside core's in the OpenAPI catalogue. Each listed directory is scanned (recursively, at container build time) for classes implementing core's DescribedErrorInterface, and each one found becomes a named error component a generated client can dispatch on. Nothing outside these paths is scanned — the default is none — so an exception reaches your published contract only when you point at the directory it lives in. For a class no scan reaches, register an ErrorCatalogSourceInterface service (core ships ClassListErrorSource); autoconfiguration tags it.
 *         paths?: list<scalar|Param|null>,
 *     },
 *     pagination?: array{ // Tuning for the server's default page-based paginator. See core docs/pagination.md.
 *         max_per_page?: int|Param, // Cap the client-controlled page[size]/page[limit] of the server default paginator (closes a page-size DoS vector). The default is core's PagePaginator::DEFAULT_MAX_PER_PAGE (100); set 0 to disable the cap (unlimited). A resource with its own pagination() sets its own cap via withMaxPerPage(). // Default: 100
 *     },
 *     doctrine?: array{ // Tuning for the reference Doctrine data provider.
 *         window_functions?: bool|Param, // Use SQL window functions (ROW_NUMBER/COUNT OVER) for the bounded windowed-include batch — ONE native query per relation fetches only ~limit rows per parent and the REAL per-parent total, instead of materialising every parent's whole related set. Default true. Requires MySQL>=8, MariaDB>=10.2, SQLite>=3.25, or any PostgreSQL. On an older engine this throws a 500 at the first windowed include — set false to use the per-parent bounded fallback instead (M bounded LIMIT queries per relation, no window functions). // Default: true
 *     },
 *     defaults?: array{ // Global default response headers applied to a resource that declares none (bundle ADR 0054). A resource-level cacheHeaders / deprecation overrides (and merges with) these.
 *         cache_headers?: array{ // Default HTTP cache directives for safe (GET) successful reads: max_age/s_maxage/public/private/no_cache/must_revalidate/vary. Applied only when a resource declares no cacheHeaders of its own.
 *             max_age?: int|Param, // Default: null
 *             s_maxage?: int|Param, // Default: null
 *             public?: bool|Param|null, // Default: null
 *             private?: bool|Param|null, // Default: null
 *             no_cache?: bool|Param|null, // Default: null
 *             must_revalidate?: bool|Param|null, // Default: null
 *             vary?: list<scalar|Param|null>,
 *         },
 *         deprecation?: scalar|Param|null, // Default Deprecation header (IETF Deprecation-header draft): a bool (true => bare header) or a date string. Applied when a resource declares no deprecation. // Default: null
 *         sunset?: scalar|Param|null, // Default RFC 8594 Sunset HTTP-date. Applied when a resource declares no sunset. // Default: null
 *         sunset_link?: scalar|Param|null, // Default URI for the companion Link: <uri>; rel="sunset" (emitted only when a sunset is set). // Default: null
 *     },
 *     servers?: array<string, array{ // Default: []
 *         base_uri?: scalar|Param|null, // Default: null
 *         version?: scalar|Param|null, // Default: null
 *     }>,
 *     atomic_operations?: array{ // The JSON:API Atomic Operations extension (https://jsonapi.org/ext/atomic): a batch of write operations applied in order, all-or-nothing, within one request. Opt-in (default off): when enabled, each server gains a POST {path} endpoint that negotiates the atomic ext, parses atomic:operations, and runs them transactionally with local-id (lid) cross-references.
 ...
 *         describedby?: bool|Param, // Stamp a top-level links.describedby onto every JSON:API response pointing at the served OpenAPI document for the request's server (JSON:API 1.1). Only takes effect when the document routes are actually served (generation enabled + the expose gate). Default true. // Default: true
 *         multi_server?: "per_server"|"combined"|Param, // per_server (default): one document per server, served at /{server}/docs.json. combined: a single document spanning every server at the json path only (D5). // Default: "per_server"
 *         enum_value_descriptions?: "both"|"extensions"|"description"|Param, // How a backed enum's per-value descriptions are surfaced (D16): both (markdown table + x-enum-* extensions), extensions (only the extensions), or description (only the markdown table). Default both. // Default: "both"
 *         public_path?: scalar|Param|null, // Also emit a fully static <server>.json (+ .yaml when symfony/yaml is installed) into this directory at cache:warmup (D17), so a web server/CDN can serve the document with zero PHP. Null = controller-only (served from the cache dir). // Default: null
 *         json?: array{
 *             path?: scalar|Param|null, // The path the default server's document is served at. Default /docs.json. // Default: "/docs.json"
 *         },
 *         json_schema?: array{ // Serve the standalone per-type JSON Schema 2020-12 documents over HTTP, aggregated into one object keyed by type, alongside the OpenAPI document (the source for a client codegen's opt-in validation seam). Behind the same expose gate as the document. The CLI export (json-api:json-schema:export) stays available regardless.
 *             enabled?: bool|Param, // Register the JSON Schema HTTP routes. Default true (still subject to the expose gate). The route serves GET /schemas.json (+ /{server}/schemas.json per server). // Default: true
 *             path?: scalar|Param|null, // The path the default server's aggregate schema document is served at. Default /schemas.json. // Default: "/schemas.json"
 *         },
 *         ui?: array{ // The Swagger UI / ReDoc documentation viewer (design D6, ADR 0079): a single config-driven route rendering plain CDN-linked HTML, behind the same expose gate as the document plus `ui.enabled`.
 *             enabled?: bool|Param, // Serve the viewer route. Default true (still subject to the expose gate). // Default: true
 *             renderer?: "swagger"|"redoc"|Param, // swagger (default) or redoc — one, not both. // Default: "swagger"
 *             path?: scalar|Param|null, // The viewer route path. Default /docs. // Default: "/docs"
 *             cdn?: scalar|Param|null, // Override the pinned CDN asset origin (self-host / air-gap). Null = the bundle-pinned jsDelivr URL. // Default: null
 *         },
 *         info?: array{ // The OpenAPI info block (G4/G5). Title/version default to JSON:API / 1.0.0 per server when omitted.
 ...
 *             license?: array{
 *                 name?: scalar|Param|null, // Default: null
 *                 identifier?: scalar|Param|null, // Default: null
 *                 url?: scalar|Param|null, // Default: null
 *             },
 *         },
 ...
 *             description?: scalar|Param|null, // Default: null
 *         }>,
 *         security?: array{ // Named security schemes + the document-level default requirement applied to operations carrying a security expression (D8). The authz expression itself is never parsed for scheme semantics.
 *             schemes?: array<string, array{ // Default: []
 *                 type?: scalar|Param|null,
 *                 description?: scalar|Param|null, // Default: null
 *                 scheme?: scalar|Param|null, // Default: null
 *                 bearerFormat?: scalar|Param|null, // Default: null
 *                 in?: scalar|Param|null, // Default: null
 *                 apiKeyName?: scalar|Param|null, // The header/query/cookie key name for an apiKey scheme. // Default: null
 *                 openIdConnectUrl?: scalar|Param|null, // Default: null
 *                 flows?: array{ // The supported OAuth2 flows (only for type: oauth2). Each flow: authorizationUrl/tokenUrl/refreshUrl as its grant requires + scopes (name => description).
 *                     implicit?: array{
 *                         authorizationUrl?: scalar|Param|null, // Default: null
 *                         tokenUrl?: scalar|Param|null, // Default: null
 *                         refreshUrl?: scalar|Param|null, // Default: null
 ...
 *                 },
 *             }>,
 *             default_requirement?: string|list<string|array{ // Default: []
 *                 name?: scalar|Param|null,
 *                 scopes?: list<scalar|Param|null>,
 *             }>,
 *         },
 *         externalDocs?: array{ // Document-level external documentation link.
 *             url?: scalar|Param|null,
 *             description?: scalar|Param|null, // Default: null
 *         },
