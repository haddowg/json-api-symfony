 ...
 * related to-many, bundle ADR 0052 / core ADR 0057) from an unpaginated one: both
 * carry a `null` total, but a count-free windowed result was sliced to a page and
 * must render a count-free page — `meta.page`/`links` without `total`/`last`, the
 * `next` link driven by {@see $hasMore} (the data layer typically fetches one item
 * past the window to set it) — so the page is built via
