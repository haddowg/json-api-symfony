...
/**
 * The in-memory half of the extensible-handler-seam conformance pair: serves the
 * {@see RelationCountArticleResource} (a custom count filter + count sort) over the
 * canonical fixtures, with the `articles` provider wired with the in-memory
 * demonstrator arms ({@see ArticleProviderFactory::createArticlesWithCountArms}) so
 * the custom value objects execute. Distinct cache/project dirs keep its compiled
 * container from colliding with the other kernels'.
 */
final class RelationCountArmInMemoryTestKernel extends Kernel
{
    ...
    {
        $dir = \sys_get_temp_dir() . '/json-api-symfony-tests/relation-count-arm-app';
    ...
    {
        return \sys_get_temp_dir() . '/json-api-symfony-tests/relation-count-arm-cache/' . $this->environment;
    }

    public function getLogDir(): string
    {
        return \sys_get_temp_dir() . '/json-api-symfony-tests/relation-count-arm-log';
    }
    ...
    protected function configureContainer(ContainerConfigurator $container, LoaderInterface $loader, ContainerBuilder $builder): void
    {
        ...
        $services->set(RelationCountArticleResource::class);
        ...
        $services->set('test.articles_provider', InMemoryDataProvider::class)
            ->factory([ArticleProviderFactory::class, 'createArticlesWithCountArms'])
